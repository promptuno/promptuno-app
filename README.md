...
Promptuno is one product with two surfaces: the web app and the Chrome extension. It has two core modes: Prompt mode for better AI prompts, and Write mode for polished usable writing.
...
The extension keeps the same 5 free generations rule, uses the same primary models (ChatGPT, Claude, Gemini, Copilot), supports Prompt mode and Write mode, supports insertion into editable fields, and stores prompt history/saved prompts locally. Chrome Web Store copy is included in `extension/STORE_LISTING.md`.
